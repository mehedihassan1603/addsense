...
const Premium = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axiosPublic.get("/premium");
  ...
  return (
    <div className="w-3/4 mx-auto md:w-1/5">
      {basic.map((plan, index) => (
        <div key={index} className="relative bg-rose-500 text-white p-6 ">
            ...
            <div>
              ...
              >
                <div className="bg-amber-400 p-10 space-y-10 rounded-lg">
                  <div>
                  <Link className="text-black border-2 w-full border-rose-600 rounded-md px-4 py-2 hover:bg-rose-600 hover:text-white" to="/premiumpay">
                    Payment Manually
                  </Link>
                  </div>
                  <div>
