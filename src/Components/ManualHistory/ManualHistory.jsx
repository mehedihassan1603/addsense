...
const ManualHistory = () => {
  ...
  const handleAccept = async (id, amount, userEmail) => {
    ...
    try {
      const response = await axiosPublic.put(`/number/${id}`, { status: updatedStatus });
        ...
        <tbody>
          {paymentHistory.map((payment, index) => (
            <tr key={index}>
              ...
              <td className="border border-gray-300 px-4 py-2">{moment().format('MMMM Do YYYY, h:mm:ss a')}</td>
              <td className="border border-gray-300 px-4 py-2">
              {payment.status === "pending" && (
                    <>
                      <button
                        onClick={() => handleAccept(payment._id, payment.amount, payment.userEmail)}
